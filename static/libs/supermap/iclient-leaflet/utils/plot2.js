...
define(function (require, exports, module) {
  ...
  plot.init = function (cfg) {
    plot.pLayer = L.supermap.plotting.plottingLayer('plot', cfg.url);// 标绘图层
    plot.pLayer.addTo(cfg.map);
    plot.pDraw = L.supermap.plotting.drawControl(plot.pLayer);
    plot.pDraw.addTo(cfg.map);
    plot.pEdit = L.supermap.plotting.editControl();
    plot.pEdit.addTo(cfg.map);
    // plot.pEdit
    if (cfg.callback) {
      plot.initPlotPanel(cfg);
    }
    plot.initStylePanel(cfg);
    plot.plotting = L.supermap.plotting.getControl(cfg.map, cfg.url);
    //console.log(plot.plotting.getGOAnimationManager());
    plot.goAnimationManager = plot.plotting.getGOAnimationManager();
    setInterval(() => {
      plot.execute();
    }, 100);
  };
   ...
   * @param cfg
   */
  plot.initPlotPanel = function (cfg) {
    if (plot.verifyCfg(cfg, 'initPlotPanel')) {
      let symbolLibManager = L.supermap.plotting.symbolLibManager(cfg.url);
      if (symbolLibManager.isInitializeOK()) {
        if (cfg.callback) {
          cfg.callback(symbolLibManager.getSymbolLibByIndex(0));
        }
      } else {
        symbolLibManager.on(SuperMap.Plot.Event.initializecompleted, function (result) {
          if (result.libIDs.length !== 0) {
            if (cfg.callback) {
              cfg.callback(symbolLibManager.getSymbolLibByIndex(1));
            }
          }
        });
   ...
   * @param cfg
   */
  plot.initStylePanel = function (cfg) {
    if (plot.verifyCfg(cfg, 'initStylePanel')) {
      plot.pEdit.on(SuperMap.Plot.Event.featuresselected, function (event) {
        cfg.setStylePanel(event.features);
      });
      plot.pEdit.on(SuperMap.Plot.Event.featuresmodified, function (event) {
        cfg.setStylePanel(event.features);
      });
      plot.pEdit.on(SuperMap.Plot.Event.featuresunselected, function (event) {
        cfg.setStylePanel(event.features);
      });
    } else {
      console.log('参数不完整')
   ...
   * @param changes
   */
  plot.afterModifySelectFeature = function (rowIndex, rowData, changes) {
   ...
   * @param cfg
   */
  plot.doPlot = function (cfg) {
    if (plot.pDraw !== null) {
      //plot.pDraw.handler.disable();
      plot.pDraw.handler.libID = cfg.item.libID;
      plot.pDraw.handler.code = cfg.item['symbolCode'];
      plot.pDraw.handler.serverUrl = cfg.url;
      plot.pDraw.handler.enable();
    }
  };
   ...
   * 保存态势图
   */
  plot.saveSimulationMap = function () {
    plot.unPlot();
    plot.plotting.getSitDataManager().saveAsSmlFile("situationMap2", function (res) {
      console.log(res);
    });
  };
   ...
   * 加载态势图
   */
  plot.loadSimulationMap = function () {
    plot.plotting.getSitDataManager().openSmlFileOnServer('situationMap2', function (evt) {
      plot.pDraw.setDrawingLayer(evt.sitDataLayers[0]);
    });
  };
   ...
   * 删除选中标号
   */
  plot.deleteSymbol = function () {
    plot.pEdit.deleteSelectedFeatures();
  };

  /**
   * 清空绘制
   */
  plot.clearLayer = function () {
    plot.unPlot();
    for (let i = 0; i < map.getPlottingLayers().length; i++) {
   ...
   */
  plot.play = function () {
    if (null === plot.goAnimationManager.goAnimations) {
      return;
    }
    for (let i = 0; i < plot.goAnimationManager.goAnimations.length; i++) {
      plot.goAnimationManager.goAnimations[i].play();
    }
    console.log(plot.goAnimationManager.goAnimations);
  };
  ...
  plot.pause = function () {
    ...
    for (let i = 0; i < plot.goAnimationManager.goAnimations.length; i++) {
      plot.goAnimationManager.goAnimations[i].pause();
    }
  };
  ...
  plot.stop = function () {
    ...
    for (let i = 0; i < plot.goAnimationManager.goAnimations.length; i++) {
      plot.goAnimationManager.goAnimations[i].stop();
    }
  };
  ...
  plot.reset = function () {
    ...
    for (let i = 0; i < plot.goAnimationManager.goAnimations.length; i++) {
      plot.goAnimationManager.goAnimations[i].reset();
    }
  };

  plot.execute = function () {
    plot.goAnimationManager.execute();
  };

  plot.deleteSelectedFeaturesAnimation = function () {
    if (null === plot.goAnimationManager.goAnimations) {
      return;
    }
    if (0 === plot.pEdit.getSelectedFeatures().length) {
      return;
    }
    let selectFeature = plot.pEdit.getSelectedFeatures()[0];
    let animations = plot.goAnimationManager.findGOAnimationsByFeature(selectFeature);
    for (let i = 0; i < animations.length; i++) {
      plot.goAnimationManager.removeGOAnimation(animations[i]);
    }
  };
   ...
   * 删除全部动画
   */
  plot.deleteAllAnimation = function () {
    if (null === plot.goAnimationManager.goAnimations) {
      return;
    }
    plot.goAnimationManager.reset();
    plot.goAnimationManager.removeAllGOAnimation();
  };

  /**
   * 创建动画
   */
  plot.createAnimation = function (type,startTime) {
    if (0 === plot.pEdit.getSelectedFeatures().length) {
      return;
    }
    console.log("240[startTime]:",startTime);
    let obj = plot.selectAnimationType(type);
    let feature = plot.pEdit.getSelectedFeatures()[0];
    let goAnimationNameUUid = SuperMap.Plot.PlottingUtil.generateUuid();
    let goAnimationName = obj.selectValue + goAnimationNameUUid;

    let goAnimation = plot.goAnimationManager.createGOAnimation(obj.animationType, goAnimationName, feature);
    switch (goAnimation.getGOAnimationType()) {
      case SuperMap.Plot.GOAnimationType.ANIMATION_ATTRIBUTE: {//属性动画
        goAnimation.startTime=startTime;//开始时间
      ...
      case SuperMap.Plot.GOAnimationType.ANIMATION_WAY: {//路径动画
        ...
        let arypts = [];
        let pt = L.latLng(44, 88);
        let pt1 = L.latLng(30, 91);
        let pt2 = L.latLng(37, 102);
        let pt3 = L.latLng(30, 106);
        let pt4 = L.latLng(34, 109);
        let pt5 = L.latLng(35, 114);
        let pt6 = L.latLng(40, 116);
  ...
  plot.selectAnimationType = function (type) {
    let aType;
    switch (type) {
      case '属性动画':
        aType = SuperMap.Plot.GOAnimationType.ANIMATION_ATTRIBUTE;
        break;
      case '闪烁动画':
        aType = SuperMap.Plot.GOAnimationType.ANIMATION_BLINK;
        break;
      case '生长动画':
        aType = SuperMap.Plot.GOAnimationType.ANIMATION_GROW;
        break;
      case '旋转动画':
        aType = SuperMap.Plot.GOAnimationType.ANIMATION_ROTATE;
        break;
      case '比例动画':
        aType = SuperMap.Plot.GOAnimationType.ANIMATION_SCALE;
        break;
      case '显隐动画':
        aType = SuperMap.Plot.GOAnimationType.ANIMATION_SHOW;
        break;
      case '路径动画':
        aType = SuperMap.Plot.GOAnimationType.ANIMATION_WAY;
        break;
    }
    return {selectValue: type, animationType: aType};
  };

  plot.updateSelectFeature = function (updated, selectfeatures) {
    let transaction = new SuperMap.Plot.Transaction();
    plot.plotting.getTransManager().add(transaction);
    for (let i = 0; i < selectfeatures.length; i++) {
      let transInfo = new SuperMap.Plot.TransactionInfo();
      ...
      if (updated != null) {
        switch (updated['NAME']) {
          ...
          case pConf.displayTextContentName[0]:
            /*if (selectfeatures[i].symbolType === SuperMap.Plot.SymbolType.SYMBOLTEXT) {
              ...
              let updatedValueStr = updated.value;
              let textContent = updatedValueStr.split(",");
        ...
        if (updated.group === pConf.group[8]) {
          ...
          if (updated['NAME'] === pConf.displayName[3]) {
            let code = parseInt(updated.value);
            if (selectfeatures[i].symbolType === SuperMap.Plot.SymbolType.NODECHAIN && code != null) {
              let symbolLibManager = L.supermap.plotting.symbolLibManager(serverUrl);
              let subCode = symbolLibManager.findSymbolByCode(code);
  ...
  plot.collectionPropertyGridRows = function (features) {
    let rows = [];
    if (features.length === 0) {
      return [];
    }
    let dotSelectFeatures = [];
    let algoSelectFeatures = [];
    let sameFeatures = [];
    let otherFeatures = [];
    ...
    let selectfeature = selectfeatures[0];
    ...
    let annotationRows = getAnnotationRows(selectfeature);
    let relLineTextRows = getRelLineTextRows(selectfeature.relLineText);
    let symbolRankRows = getSymbolRankRows(selectfeature);
    let surroundLineTypeRows = getSurroundLineTypeRows(selectfeature.symbolType);
    let displayRows = getDisplayRows();
    let showRows = getShowRows();
    let fillGradinetRows = getFillGradientModeRows();
    let arrowTypeStart = getArrowTypeRows(selectfeature);
    let arrowTypeEnd = getArrowTypeRows(selectfeature);
    let radiusTypeRows = getRadiusTypeRows(selectfeature);
    let lineStyleRows = getLineStyleRows();
    let routeNodeTypeRows = getRouteNodeTypeRows();
    let positionOffsetTypeRows = getPositionOffsetTypeRows(); //偏移线类型
    let textBoxTypeRows = getTextBoxTypeRows();
    let lineMarkingTypeRows = getLineMarkingTypeRows();
    let arrowHeadTypeRows = getArrowHeadTypeRows();
    let arrowBodyTypeRows = getArrowBodyTypeRows();
    let arrowTailTypeRows = getArrowTailTypeRows();
    let lineRelationTypeRows = getLineRelationTypeRows();
    let subSymbolsTypeRows = getSubSymbolsTypeRows(selectfeature);
    //基本0：
    //可见性
    let visibilityObj = {};
...
//0--无箭尾，1--直线,3--双线
   ...
   * 验证必填项
   */
  plot.verifyCfg = function (cfg, fnc) {
